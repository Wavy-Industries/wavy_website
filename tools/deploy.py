...
import hashlib, json, os, io, pathlib, subprocess, threading
...
PARALLEL   = int(os.getenv('DEPLOY_PARALLEL', '4'))
CACHE_PATH = pathlib.Path(__file__).resolve().parent / '.deploy-hash-cache.json'
...
def build_hash_map(base: pathlib.Path) -> dict[str, str]:
    # Load mtime+size cache so unchanged files skip the SHA-256 pass
    try:
    ...
    for p in base.rglob("*"):
        if p.is_file() and p.name not in excluded_files:
            ...
            if cached and cached["m"] == st.st_mtime and cached["s"] == st.st_size:
                h = cached["h"]
                hits += 1
            else:
                h = sha256sum(p)
            out[rel] = h
            new_cache[rel] = {"h": h, "m": st.st_mtime, "s": st.st_size}

    CACHE_PATH.write_text(json.dumps(new_cache))
    print(f"Local files: {len(out)} ({hits} cached, {len(out) - hits} hashed)")
    return out
...
_pool_connections: list[FTP_TLS] = []
_pool_lock = threading.Lock()

def _get_thread_ftp() -> FTP_TLS:
    if not hasattr(_tls, 'ftp'):
        ftp = make_ftp()
        _tls.ftp = ftp
        with _pool_lock:
            _pool_connections.append(ftp)
    return _tls.ftp

def _close_pool():
    for ftp in _pool_connections:
        try:
            ftp.quit()
        except Exception:
            pass
...
def parallel_upload(to_upload: list[str]):
    n = min(PARALLEL, len(to_upload))

    def worker(rel: str):
        ftp = _get_thread_ftp()
        ensure_remote_dirs(ftp, rel)
        with (LOCAL_DIR / rel).open("rb") as f:
            ftp.storbinary(f"STOR {rel}", f)
        print(f"  PUT       {rel}")

    failed = []
    with ThreadPoolExecutor(max_workers=n) as pool:
        futures = {pool.submit(worker, rel): rel for rel in to_upload}
    ...
    _close_pool()
    if failed:
        raise RuntimeError(f"{len(failed)} upload(s) failed")
...
def main():
    ...
    # parallel upload
    if to_upload:
        print(f"  Uploading {len(to_upload)} file(s) with {min(PARALLEL, len(to_upload))} connections...")
        parallel_upload(to_upload)

    # delete (sequential on control connection)
