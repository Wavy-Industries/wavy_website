export class Log {
  ...
  #print(message: any, color: string) { if (import.meta.env.PROD && Log.#LEVEL_PROD > this.#level) return; if (typeof message === 'object') { console.log(`%c[ ${this.#name} ] ⬇︎`, `color: ${color}`); console.log(message); return; } console.log(`%c[ ${this.#name} ] ${message}`, `color: ${color}`); }
