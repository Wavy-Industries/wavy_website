...
import midiPlayerSectionsImg from '~/assets/monkey/monkey_midi_player_overview.png'
...
## MIDI effects
MONKEY has in total 6 MIDI effects each with 10 presets or banks. Similarly to HOLD, the effects are toggled on and off. Only one effect might be enabled at once. 

- If you enable a new effect and a previous effect was enabled, this new effect will be enabled and the previous disabled.
- If you press on an effect which is already enabled, it will disable it. 

Once an effect is enabled, you can change its preset/bank. This is simply done by directly pressing the numbered keys together with SHIFT.
...
### PAT and DRM
<img src={midiPlayerSectionsImg.src} alt="MONKEY PAT and DRM overview" />

The PAT and DRM effect are user programmable. Both let you upload arbitrary MIDI. However DRM is speficically targetted towards drum loops. It will send on MIDI channel 10 when enabled.

Instead of presets, these effects have in total 10 banks of MIDI looops. Each bank can hold 15 MIDI loops which in total yields 150 MIDI loops for both PAT and DRM.

You can customise the loops through our device utility page on our website.
